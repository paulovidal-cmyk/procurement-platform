...
function tcpsId(n) {
  return `TCPS-${String(n).padStart(4, '0')}`
}
...
const SEED_CARDS = [
  {
    id: 'card-1', cardId: 'TCPS-0001',
    fornecedor: 'TechSupply Ltda', cnpj: '11.222.333/0001-81',
    razaoSocial: 'TechSupply Comercio e Servicos Ltda',
    categoria: 'TI', tipoBaseline: 'Histórico',
    ...
    comprador: 'Ana Lima', compradorId: 'u1',
    columnId: 'coordenacao', approvalLevel: 'coordenacao', approvalLevelLabel: 'Coordenação',
  ...
  {
    id: 'card-2', cardId: 'TCPS-0002',
    fornecedor: 'LogisBrasil S.A.', cnpj: '22.333.444/0001-97',
    razaoSocial: 'LogisBrasil Transportes S.A.',
    categoria: 'Logística', tipoBaseline: 'Orçamento',
    ...
    comprador: 'Ana Lima', compradorId: 'u1',
    columnId: 'gestor', approvalLevel: 'gestor', approvalLevelLabel: 'Gestor',
    isLocked: true,
    approvalHistory: [
      { actor: 'Bruno Costa', action: 'aprovado', comment: 'Proposta dentro do orçamento.', at: new Date(Date.now() - 2 * 86400000).toISOString() },
    ],
  ...
  {
    id: 'card-3', cardId: 'TCPS-0003',
    fornecedor: 'SoftMaker ME', cnpj: '33.444.555/0001-07',
    razaoSocial: 'SoftMaker Desenvolvimento de Software ME',
    categoria: 'TI', tipoBaseline: 'MPE',
    ...
    comprador: 'Ana Lima', compradorId: 'u1',
    columnId: 'aguardando', approvalLevel: 'coordenacao', approvalLevelLabel: 'Coordenação',
  ...
  {
    id: 'card-4', cardId: 'TCPS-0004',
    fornecedor: 'GlobalParts Inc', cnpj: '44.555.666/0001-13',
    razaoSocial: 'GlobalParts Importacao e Comercio Ltda',
    categoria: 'Materiais', tipoBaseline: 'Histórico',
    ...
    comprador: 'Ana Lima', compradorId: 'u1',
    columnId: 'diretor', approvalLevel: 'diretor', approvalLevelLabel: 'Diretor',
    isLocked: true,
    approvalHistory: [
  ...
  {
    id: 'card-5', cardId: 'TCPS-0005',
    fornecedor: 'FornecedorXYZ', cnpj: '55.666.777/0001-29',
    razaoSocial: 'FornecedorXYZ Comercio Ltda',
    categoria: 'Facilities', tipoBaseline: 'Orçamento',
    ...
    comprador: 'Ana Lima', compradorId: 'u1',
    columnId: 'aprovado', approvalLevel: 'coordenacao', approvalLevelLabel: 'Coordenação',
    isLocked: true,
  ...
  {
    id: 'card-6', cardId: 'TCPS-0006',
    fornecedor: 'ServicosMais', cnpj: '66.777.888/0001-35',
    razaoSocial: 'ServicosMais Prestacao de Servicos Ltda',
    categoria: 'Facilities', tipoBaseline: 'Histórico',
    ...
    comprador: 'Ana Lima', compradorId: 'u1',
    columnId: 'cancelado', approvalLevel: 'coordenacao', approvalLevelLabel: 'Coordenação',
    isLocked: true,
  ...
  {
    id: 'card-7', cardId: 'TCPS-0007',
    fornecedor: 'FastMicro', cnpj: '77.888.999/0001-41',
    razaoSocial: 'FastMicro Tecnologia ME',
    categoria: 'TI', tipoBaseline: 'MPE',
    ...
    comprador: 'Ana Lima', compradorId: 'u1',
    columnId: 'aprovado', approvalLevel: 'fast_track', approvalLevelLabel: 'Fast Track',
    isLocked: true,
...
const SEED_CUSTOM_FIELDS = [
  ...
  },
  {
    ...
    key: 'tipo_contrato',
    type: 'select',
    options: ['Spot', 'Recorrente', 'Frame Agreement', 'Projeto'],
    required: false,
    order: 3,
  },
]
...
const useAppStore = create(
  persist(
    (set, get) => ({
      ...
      // ── Auth ──────────────────────────────────────────────
      login: (email) => {
        const { allUsers, pendingUsers } = get()
        const lc = email.toLowerCase().trim()

        if (!lc.endsWith('@stone.com.br')) {
          return { success: false, error: 'Acesso restrito a e-mails @stone.com.br' }
        }
        ...
        if (pendingUsers.find(u => u.email.toLowerCase() === lc)) {
          return { success: false, pending: true, error: 'Solicitação já enviada. Aguarde aprovação do Administrador.' }
        }
        ...
        const pendingUser = {
          id: 'pending-' + uuid(),
          email: lc,
          name: lc.split('@')[0].replace('.', ' ').replace(/\b\w/g, c => c.toUpperCase()),
          role: 'comprador',
          avatar: lc.slice(0, 2).toUpperCase(),
          requestedAt: new Date().toISOString(),
        }
        set(s => ({
          pendingUsers: [...s.pendingUsers, pendingUser],
          notifications: [
            {
              id: notifId(),
              type: 'new_user',
              title: 'Novo usuário aguardando aprovação',
              message: `${pendingUser.email} solicitou acesso à plataforma.`,
              cardId: null,
              targetRoles: ['admin'],
              targetUserId: null,
              isRead: false,
        ...
        return { success: false, pending: true, error: null }
      },
          ...
          notifications: [
            {
              ...
              message: `Seu acesso foi aprovado. Bem-vindo(a), ${newUser.name}!`,
              cardId: null,
              targetRoles: [],
      ...
      createCard: (formData) => {
        ...
        const card = {
          ...
          isNegative: saving.isNegative,
          columnId: route.columnId,
          approvalLevel: route.level,
          approvalLevelLabel: route.label,
          isLocked: !isFastTrack, // locked immediately when going to approval
          ...
          cards: s.cards.map(c =>
            c.id === id
              ? {
                  ...
                  approvalHistory: [
                    ...c.approvalHistory,
                    { actor: currentUser.name, action: 'aprovado', comment, at: new Date().toISOString() },
                  ],
                  updatedAt: new Date().toISOString(),
                }
              : c
          ...
          cards: s.cards.map(c =>
            c.id === id
              ? {
                  ...
                  approvalHistory: [
                    ...c.approvalHistory,
                    { actor: currentUser.name, action: 'rejeitado', comment, at: new Date().toISOString() },
                  ],
                  updatedAt: new Date().toISOString(),
                }
              : c
    ...
    }),
    {
      ...
      onRehydrateStorage: () => (state) => {
        ...
        // Graceful migration: add customFields if missing (v2 → v3 upgrade)
        if (!state.customFields) {
          state.customFields = SEED_CUSTOM_FIELDS
        }
      },
    }
  )
